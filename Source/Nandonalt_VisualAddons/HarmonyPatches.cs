	...
	public static class HarmonyPatches
	{
		...
		static HarmonyPatches()
		{
			foreach (BiomeDef biomeDef in DefDatabase<BiomeDef>.AllDefs)
			{
				if (biomeDef != null)
				{
					WeatherCommonalityRecord weatherCommonalityRecord = new WeatherCommonalityRecord();
					weatherCommonalityRecord.weather = WeatherDef.Named("Cloudy");
					weatherCommonalityRecord.commonality = 5f;
			...
			Traverse.Create(WeatherDef.Named("Cloudy")).Field("workerInt").SetValue(new WeatherWorker(WeatherDef.Named("Cloudy")));
